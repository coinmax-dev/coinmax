...
```
用户 USDT → swap USDC → Receiver Server(0xe193)
  → 前端回调 → DB 记录
  → Engine:
    1. 铸造节点 NFT (Soulbound) 给用户 (链上凭证)
    2. VaultV4.createPosition(cUSD配资) (链上记账)
    3. DB 记录节点信息

MAX 节点: $600 USDC → 铸造 $6,000 cUSD 配资 | 120天 | 0.9%/日 = 54U/日
MINI 节点: $100 USDC → 铸造 $1,000 cUSD 配资 | 90天 | 0.9%/日 = 9U/日

Engine 基于 DB 数据一次性铸造 (NFT + cUSD + 记录)
```
...
**激活后第二天开始按 $6,000 × 0.9% = $54/日 铸造 MA**

### 4.3 大节点达标考核 (DB 判断)

```
第1天:  查看是否激活节点
  ├─ 已激活: 1-15天 每日 $54 铸造 MA → 待释放
  └─ 未激活: 不启动收益

第15天: 考核是否达标 V1
  ├─ V1达标: 16-30天 每日 $54 铸造 MA
  └─ V1不达标: 16-30天 收益暂停

第30天: 考核是否达标 V2
  ├─ V2达标: 31-60天 每日 $54 铸造 MA
  └─ V2不达标: 31-60天 收益暂停，等级降为实际等级

第60天: 考核是否达标 V4
  ├─ V4达标: 61-120天 每日 $54 铸造 MA
  └─ V4不达标: 61-120天 收益暂停，等级降为实际等级

第120天: 考核是否达标 V6
  ├─ V6达标: 解锁 $6,000 铸造 MA
  └─ V6不达标: 不可解锁 $6,000 铸造 MA，等级降为实际等级
```
...
**激活后第二天开始按 $1,000 × 0.9% = $9/日 铸造 MA（锁仓）**

### 4.5 小节点达标考核 (DB 判断)

```
第1天:  查看是否激活节点
  ├─ 已激活: 1-90天 每日 $9 铸造 MA → 锁仓
  └─ 未激活: 不启动收益

第30天: 考核是否达标 V2（哪日达标次日解锁）
  ├─ V2达标: 解锁1-60天锁仓收益，继续领取至90天
  └─ V2不达标: 收益不可解锁，等级降为实际等级
...
第90天: 考核是否达标 V4
  ├─ V4达标: 解锁 $1,000 铸造 MA
  └─ V4不达标: 不可解锁 $1,000 铸造 MA，等级降为实际等级
```
...
```
链上:
  NodeNFT (0x296c) → Soulbound NFT，只是凭证
  VaultV4 → cUSD 配资记录

DB:
  所有激活/达标/暂停/降级/解锁/销毁 → 数据库判断
  Engine 根据 DB 一次性铸造 MA → Release 合约
```
